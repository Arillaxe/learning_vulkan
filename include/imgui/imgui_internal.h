...
T ImClamp(T v, T mn, T mx) { return (v < mn) ? mn : (v > mx) ? mx
                                                             : v; }
template <typename T>
T ImLerp(double a, double b, float t) { return (T)(a + (b - a) * (double)t); }
template <typename T>
...
namespace ImGui
{
  ...
  IMGUI_API float ScaleRatioFromValueT(ImGuiDataType data_type, T v, T v_min, T v_max, float logarithmic_zero_epsilon, float zero_deadzone_size);
  template <typename T, typename SIGNED_T, typename FLOAT_T>
  IMGUI_API T ScaleValueFromRatioT(ImGuiDataType data_type, float t, T v_min, T v_max, float logarithmic_zero_epsilon, float zero_deadzone_size);
  template <typename T, typename SIGNED_T, typename FLOAT_T>
